...
for i, gene in enumerate(clean_genes):
    ...
    ss_map = None; sasa_dict = {}; dist_dict = {}
    try:
...
if results:
    lengths = [r[2] for r in results]; kappas = [r[3] for r in results]
    fig, ax = plt.subplots(figsize=(14,8))
    ax.scatter(lengths, kappas, c='#3daee9', alpha=0.6, s=30)
    ax.axhline(y=16, color='gold', linestyle='--', lw=2, alpha=0.8, label='kappa = 16 (8+5+3)')
    ax.set_xlabel('Protein Length (residues)'); ax.set_ylabel('kappa = mean depth / J_NAT')
    ax.set_title(f'Pan-Protein Vulnerability Map: {len(results)} proteins')
    ax.legend(); ax.grid(True, alpha=0.3)
    plt.tight_layout(); plt.savefig('kappa_clean_proteins.png', dpi=150); plt.show()
