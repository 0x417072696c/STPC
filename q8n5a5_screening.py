"""
q8n5a5_screening.py – скрининг гипотетического белка Q8N5A5.
Предсказывает уязвимые позиции для белка с неизвестной функцией.
"""
import allymind_core, cascade, pdb_parser
import numpy as np, matplotlib.pyplot as plt, csv, time

UNIPROT = "Q8N5A5"
# Для этого белка нет экспериментальной PDB, используем предсказанную AlphaFold
PDB_ID = "AF-Q8N5A5-F1"  # AlphaFold предсказание
OUTPUT_CSV = "q8n5a5_profile.csv"
OUTPUT_PNG = "q8n5a5_vulnerability_profile.png"
...
if not pdb_file:
    print("PDB не загрузился. Пробуем без структуры (только последовательность).")
    ss_map = None
else:
...
for pos in range(1, len(seq)+1):
    ...
    structures.append(ss_map[pos-1] if ss_map else '?')
...
ax.set_title(f'Профиль уязвимости гипотетического белка {uid}\nПроверено {total_mutations} мутаций за {elapsed:.1f} с')
ax.legend()
